...
const totalFuel = (crabs:Crabs, constant_rate=true):number =>{
    const calculateRate = (fuel:number, count:number, index:number) =>{
        if(constant_rate){ return fuel + (count * index); }
        const all_indexes = Array.from(Array(index + 1).keys()).reduce((t, i)=>t+i, 0)
        return fuel + (count * all_indexes);
    };
    let fuel = crabs.left.reduce(calculateRate, 0);
    fuel = crabs.right.reduce(calculateRate, fuel);
    return fuel;
}
const findLeastFuel = (crabs:Crabs, constant_rate=true) =>{
    const least = { fuel:0, position: 0 }
    least.fuel = totalFuel(crabs, constant_rate);
    for(let position = 1; position <= crabs.max_position; position ++){
        crabs = shiftFromPosition(crabs);
        const current_fuel = totalFuel(crabs, constant_rate);
...
const testPart2 = async (input:string):Promise<boolean> =>{
    const puzzle_input = await puzzle.parseInput(input);
    const crabs = parseCrabs(puzzle_input.blocks[0][0]);
    const least = findLeastFuel(crabs, false);
    return least.fuel === 168;
}
